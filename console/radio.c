 ...
 *
 * test tool for bttv + WinTV/Radio 3.88
 *
 */
 ...
 *             signal strength.
 * 14 Aprl 03  - Grant Petersen: <grant.petersen@clear.net.nz>
 *             Added -l option to print current frequency to stdout.
 *             Intended for use with GCI or perl control of the radio.
 */
...
static int radio_getfreq(int fd, float *freq)
{
    ...
    if (ioctl_status == -1)
        return ioctl_status;
    *freq = (ifreq/freqfact) + (float)((ifreq%freqfact*20+freqfact/2-1 )/freqfact)/20.0 ;
    return 0;
}
...
static int
find_index(int ifreq)
{
    int i;

    for (i = 0; i < stations; i++) {
      if ((ifreq > freqs[i] - FREQ_STEP) && (ifreq < freqs[i] + FREQ_STEP))
	    return i;
    }
    return -1;
}
...
usage(FILE *out)
{
    fprintf(out,
	    ...
	    "  -l       print current frequency to stdout\n"
	    "  -m       mute radio\n"
...
main(int argc, char *argv[])
{
    ...
    int    fd,key=0,done,i,ifreq = 0,lfreq=0,lastfreq = 1, mute=1;
    char   *name;
    ...
    /* parse args */
    for (;;) {
	c = getopt(argc, argv, "mhiqldsSf:c:");
	if (c == -1)
	    break;
	switch (c) {
	case 'm':
    ...
    /* GPP print current frequency to stdout then quit*/
    if (lfreq){
	...
	case 'G':
	    ...
	    mvwprintw(wcommand,1,2,"%-50s", "GO: Enter frequency: ");
	    curs_set(1);
	    echo();
	    wrefresh(wcommand);
	    ...
	    noecho();
	    curs_set(0);
	    halfdelay(1); getch(); cbreak();  // don't use echo() with getch.
	    wrefresh(wcommand);
	    if ((newfreq >= FREQ_MIN/1e6) && (newfreq <= FREQ_MAX/1e6) )
		ifreq = newfreq * 1000000;
	    else
		mvwprintw(wcommand, 1, 2, "%-50s",
	...
	case KEY_UP:
            ...
            if (ifreq > FREQ_MAX)
		ifreq = FREQ_MIN;
	    mvwprintw(wcommand, 1, 2, "%-50s", "Increment frequency");
	    break;
	case KEY_DOWN:
            ifreq -= FREQ_STEP;
            if (ifreq < FREQ_MIN)
		ifreq = FREQ_MAX;
	    mvwprintw(wcommand, 1, 2, "%-50s", "Decrease frequency");
	    break;
	case KEY_PPAGE:
	case KEY_NPAGE:
	case ' ':
