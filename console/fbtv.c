...
static struct ng_process_handle *p;
static int dx,dy;
...
static void do_capture(int from, int to, int tmp_switch)
{
	...
	if (0 != ng_grabber_setformat(&devs.video,&fmt,1)) {
	    gfmt = fmt;
	    if (NULL == (conv = ng_grabber_findconv(&devs.video,&gfmt,0))) {
		fprintf(stderr,"can't fint useful capture format\n");
		exit(1);
	    }
	    p = ng_conv_init(conv,&gfmt,&fmt);
	}
	...
	if (matrox) {
	    ...
	    devs.video.v->overlay(devs.video.handle,&off,0,starty,NULL,0,0);
	    gfx_scaler_on(starty*off.bytesperline,off.bytesperline,
			  off.width,off.height,
			  dx,dx+fmt.width,
			  dy,dy+fmt.height);
	} else {
	    devs.video.v->overlay(devs.video.handle,&fmt,dx,dy,NULL,0,1);
	}
	break;
    }
}
...
{
    int             key,c,gray=0,rc,vt=0,fps=0,t1,t2,lirc,js,err,mute=1,fdmax;
    unsigned int    ui;
    struct timeval  tv;
    time_t          t;
	...
	if ((devs.video.flags & CAN_TUNE)
	    && 0 != (freq = devs.video.v->getfreq(devs.video.handle))) {
	...
	for (;!sig;) {
		...
		fps++;
		/* grab + convert frame */
		if (NULL == (buf = ng_grabber_grab_image(&devs.video,0))) {
		    fprintf(stderr,"capturing image failed\n");
		    exit(1);
		}
		if (p) {
		    ng_process_put_frame(p,buf);
		    buf = ng_process_get_frame(p);
		}
		/* blit frame */
		dst = fb_mem +
