...
vbi_subpage_menu(struct vbi_window *vw)
{
    ...
    /* delete children */
    XtVaGetValues(vw->sub_menu,XtNchildren,&children,
	...
	XtVaSetValues(vw->sub_btn,XtNsensitive,True,NULL);
	...
	XtVaSetValues(vw->sub_btn,XtNsensitive,False,NULL);
    }
}
...
vbi_new_cb(Widget widget, XtPointer clientdata, XtPointer call_data)
{
    ...
    shell = vbi_create_widgets(XtDisplay(widget),vw->vbi);
...
{
    struct vbi_state *vbi = client;
    int pid = atoi(XtName(widget));

    vbi_capture_dvb_filter(vbi->cap,pid);
}

static Widget vbi_dvb_get_widget(Widget menu, struct psi_program *pr)
{
    WidgetList children;
    Cardinal i,nchildren;
    int pid;
...
static void vbi_dvb_update_cb(Widget widget, XtPointer client, XtPointer call)
{
    struct vbi_window  *vw = client;
    struct psi_program *pr = call;
    char name[8],label[128];
    XmString str;
    Widget push;

    if (0 == pr->t_pid)
	return;
    push = vbi_dvb_get_widget(vw->st_menu, pr);
    if (NULL == push) {
	sprintf(name,"%d",pr->t_pid);
	push = XtVaCreateManagedWidget(name, xmPushButtonWidgetClass,
				       vw->st_menu, NULL);
	XtAddCallback(push,XmNactivateCallback,vbi_dvb_pid_cb,vw->vbi);
	if (0 == vw->vbi->pid)
	    vbi_set_pid(vw->vbi, pr->t_pid);
    }
    sprintf(label,"[pid %d] %s / %s",pr->t_pid,pr->net,pr->name);
    str = XmStringGenerate(label, NULL, XmMULTIBYTE_TEXT, NULL);
    XtVaSetValues(vw->st_btn, XtNsensitive, True, NULL);
    XtVaSetValues(push, XmNlabelString, str, NULL);
    XmStringFree(str);
}

static void vbi_dvb_delete_cb(Widget widget, XtPointer client, XtPointer call)
{
    struct vbi_window  *vw = client;
    struct psi_program *pr = call;
    Cardinal nchildren;
    ...
    if (0 == pr->t_pid)
	return;
    if (vw->vbi->pid == pr->t_pid)
	vw->vbi->pid = 0;
    push = vbi_dvb_get_widget(vw->st_menu, pr);
    XtDestroyWidget(push);

    XtVaGetValues(vw->st_menu, XtNnumChildren,&nchildren, NULL);
    if (0 == nchildren)
...
Widget vbi_create_widgets(Display *dpy, struct vbi_state *vbi)
{
    Widget shell,form,menubar,tool,menu,push,tt;
