...
struct video_handle {
    ...
    unsigned int          fmtids[2*VIDEO_FMT_COUNT];
    struct ng_video_fmt   fmt;
...
void video_gd_init(Widget widget, int use_xv, int use_gl)
{
    ...
    h->win  = widget;
    h->blit = blit_init(h->win, &vinfo, use_xv, use_gl);
    blit_get_formats(h->blit,h->fmtids,DIMOF(h->fmtids));
}
...
video_gd_blitframe(struct video_handle *h, struct ng_video_buf *buf)
{
    ...
    bl = blit_get_buf(h->blit,&buf->fmt);
    if (NULL == bl)
	return -1;
    ng_copy_video_buf(bl,buf);
    blit_draw_buf(bl);
    ng_release_video_buf(bl);
    return 0;
}
...
video_gd_idle(XtPointer data)
{
    ...
    buf = ng_grabber_grab_image(&devs.video,0);
    if (NULL != buf) {
...
video_gd_start(void)
{
    ...
    if (0 != ng_grabber_setformat(&devs.video,&h->fmt,0)) {
...
video_gd_configure(int width, int height)
{
    ...
    h->fmt.width   = cur_tv_width;
    h->fmt.height  = cur_tv_height;
    h->fmt.bytesperline = 0;

    if (0 == h->fmt.fmtid) {
	ng_dev_open(&devs.video);
	for (i = 0; i < DIMOF(h->fmtids); i++) {
	    h->fmt.fmtid = h->fmtids[i];
	    if (0 == ng_grabber_setformat(&devs.video,&h->fmt,0))
		break;
	}
	if (i == DIMOF(h->fmtids))
