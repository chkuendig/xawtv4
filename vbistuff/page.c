...
static int vbi_export_html_one(wchar_t *dest, int size,
			       ...
			       int pagenr, int subnr)
{
    ...
    obuf = dest;
    link = -1;
    len = swprintf(obuf,size,L"<pre class=vt>\n");
    obuf += len; size -= len;
    for (y = 0; y < 25; y++) {
	ch = pg->text + 41*y;
	fg = -1;
	...
	for (x = 0; x <= 40; x++) {
	    ...
	    /* check for references to other subpages (2/7) */
	    ...
	    /* two-digit subpages ( 2/17, 12/17) */
	    if (subnr > 0 && y > 0 && -1 == link && x > 0 && x < 36 &&
		(iswdigit(ch[x+0].unicode) || iswspace(ch[x+0].unicode)) &&
		iswdigit(ch[x+1].unicode)  &&
		'/' == ch[x+2].unicode     &&
		iswdigit(ch[x+3].unicode)  &&
		iswdigit(ch[x+4].unicode)  &&
		!iswdigit(ch[x-1].unicode) &&
		!iswdigit(ch[x+5].unicode)) {
		if (ch[x+0].unicode == ch[x+3].unicode &&
		    ch[x+1].unicode == ch[x+4].unicode) {
		    len = swprintf(obuf,size,L"<a href=\"01.html\">");
		} else {
		    int sp;
		    sp = iswdigit(ch[x+0].unicode) ? (ch[x+0].unicode-'0')*10 : 0;
		    sp += (ch[x+1].unicode - '0' + 1);
		    len = swprintf(obuf,size,L"<a href=\"%02d.html\">", sp);
		}
		obuf += len; size -= len;
		link = 4;
	    }
...
static int vbi_export_html_links(wchar_t *dest, int size,
				 struct vbi_state *vbi, struct vbi_page *pg,
				 int pagenr, int subnr)
{
    ...
    len = swprintf(obuf,size,L"<div class=quick>\n");
    obuf += len; size -= len;
...
static int vbi_export_html(wchar_t *dest, int size,
			   ...
			   int pagenr, int subnr)
{
    struct vbi_page subpg;
    int i;
    wchar_t  *obuf = dest;
    int       len  = 0;

    /* header */
    len  = swprintf(obuf,size,page_top,pagenr,
		    (-1 == subnr) ? 0 : subnr);
    obuf += len; size -= len;
    ...
    if (-1 == subnr) {
	for (i = 1; i <= VBI_MAX_SUBPAGES; i++) {
	    if (!vbi_fetch_vt_page(vbi->dec,&subpg,pagenr,i,
				   VBI_WST_LEVEL_1p5,25,1))
		continue;
	    len = vbi_export_html_one(obuf, size, vbi, &subpg, pagenr, i);
	    obuf += len; size -= len;
	}
    } else {
	len = vbi_export_html_one(obuf, size, vbi, pg, pagenr, subnr);
	obuf += len; size -= len;
    }

    /* navigation links */
    len = vbi_export_html_links(obuf, size, vbi, pg, pagenr, subnr);
    obuf += len; size -= len;
			   ...
			   int pagenr, int subnr, enum mime_type type)
{
    ...
    case TYPE_ANSI:
	if (-1 == subnr) {
	    req->body  = malloc(TXTBUF * VBI_MAX_SUBPAGES);
	    req->lbody = 0;
	    for (i = 1; i <= VBI_MAX_SUBPAGES; i++) {
		if (!vbi_fetch_vt_page(vbi->dec,&subpg,pagenr,i,
				       VBI_WST_LEVEL_1p5,25,1))
		    continue;
		len = vbi_export_txt(req->body + req->lbody,
				     CHARSET, TXTBUF, &subpg,
				     &vbi_fullrect,
				     type == TYPE_TEXT
				     ? VBI_NOCOLOR
				     : VBI_ANSICOLOR);
    ...
    case TYPE_HTML:
	if (-1 == subnr) {
	    wpage = malloc(HTMLBUF * sizeof(wchar_t) * VBI_MAX_SUBPAGES);
	} else {
...
void buildpage(struct REQUEST *req)
{
    ...
    /* pages */
    subpage = -1;
    if (2 == sscanf(req->path,"/%3x/all.%8s",&pagenr,type) ||
	3 == sscanf(req->path,"/%3x/%2x.%8s",&pagenr,&subpage,type)) {
	t = 0;
	if (0 == strcmp(type,"html"))
	    t = TYPE_HTML;
	if (0 == strcmp(type,"txt"))
	    t = TYPE_TEXT;
	if (0 == strcmp(type,"ansi"))
	    t = TYPE_ANSI;
	if (0 == t) {
	    mkerror(req,404,1);
	    return;
